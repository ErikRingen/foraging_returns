...
- `n_samples`: Monte Carlo samples per MCMC draw (default: 1000)
- `max_group_size`: Skip groups larger than this (optional)

**Returns:** xarray Dataset with 'shapley_contribution' variable

### `shapley_contributions()`

Compute Shapley values for a single group.

**Parameters:**
- Same as above, plus `group_idx` to specify which group

**Returns:** xarray DataArray with shape (chain, draw, forager)
