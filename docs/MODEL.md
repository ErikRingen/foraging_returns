...
This document provides the mathematical specification for the foraging returns model.
...
Individual forager skill $S(x)$ depends on age $x$ via the M-K-B parametrization:
...
where $K(t, t') = \sigma^2 \exp\left(-\frac{(t-t')^2}{2\ell^2}\right)$ (squared exponential kernel).
...
$$\text{rate}_g = \frac{\text{shape}}{\mu_g}$$

The expected return $\mu_g$ depends on the aggregation method.
...
|-----------|--------------|------------|-------------|
| **Skill Curve** | | | |
| `m0` | `normal` | mu=-1, sigma=1 | Log senescence rate |
| `k0` | `normal` | mu=1, sigma=1 | Log learning rate |
| `b0` | `normal` | mu=0, sigma=1 | Log skill elasticity on learning |
| **Effort** | | | |
...
| `shape` | `gamma` | alpha=10, beta=10 | Gamma shape parameter |
| **Random Effects** | | | |
...
| `unique_date` | Unique dates in the study period |
