...
- Returns and recall combined. Foragers consume food acquired during foraging trips (Crittenden et al., 2013; Berbesque et al., 2016); excluding it underestimates production. Both streams are merged into a single per-`pooled_group` × date production record.
...
- Effort observations. For each forager × in-camp date, an effort indicator is set to 1 if the forager left camp that day and 0 otherwise. These observations enter the Bernoulli effort component.
...
- Zero-inferred returns. (forager, date) pairs with positive out-of-camp time but no recorded food package are appended as kcal = 0 rows. These rows are excluded from the LogNormal returns component, which is conditional on positive returns. Group composition is not recoverable from time-allocation alone, so each zero-inferred row is recorded as a solo trip; this misrepresents days on which the forager went out in a cooperative group that returned nothing. The zero-inferred rows are 290 of 1 360 group rows (21 %).
- Age estimation. BaYaka birth years are not recorded numerically. Age was estimated using the consensus method of Diekmann et al., with 1-year intervals for individuals under 20 and 10-year intervals for those 20+. The model's measurement-error component places a Normal observation density on the recorded age centred on the latent true age (see §4.7), so we need an observation SD. The intervals are most naturally read as half-widths around a point estimate, so we use those half-widths directly as the Normal SD: `age_sigma` = 1 yr (under 20) and 5 yr (20+). Treating the interval as Uniform instead would imply a strictly smaller SD of half-width$/\sqrt{3}$; we deliberately use the larger half-width to avoid understating age uncertainty.
- Inclusion. Three of 54 listed camp members were excluded as under age 1; foragers without any foraging activity over the study window are then dropped at the time-aggregation step (two further age-1 toddlers). 49 foragers appear in the foraging-activity log; of those, 48 contributed at least one positive-kcal food package and inform the model. The 49th forager went on at least one logged foraging trip but is missing from the in-camp daily-presence file (`daysincamp.csv`), so no effort or success observations exist for him; he occupies a slot in the forager dimension but does not enter the likelihood. Figures throughout the manuscript and supplement therefore use $n = 48$.
...
The causal structure of the model is summarized in the following directed acyclic graph:
...
where $\mathcal{S}_N$ is the set of all $n!$ orderings of $N$ and $P_\pi^i$ is the set of players preceding $i$ in $\pi$. Plain reading: imagine the foragers joining the group one at a time in a random order; $\phi_i$ is the average extra kcal that adding forager $i$ contributes when they join. Both forms are equivalent — the left sum groups orderings by which subset $S$ is already present, the right sum averages directly over orderings.
...
We compute $\phi_i(v_\theta)$ for every posterior draw $\theta^{(s)}$ and every group $g$. Because $v_\theta$ is parametrised by a single posterior draw, the computation is independent across groups and across posterior samples.

#### Per-group rescaling to observed kcal

The raw Shapley value $\phi_i(v_\theta)$ is anchored to the model's predicted median $\mu_g$, in scaled-kcal units. To convert it into a kcal share of what the group actually brought back that day, we apply three per-group scalar multiplications, in this order:
...
![Computation graph for the fit reported in the main text: effort + success + returns components; latent age; correlated (effort, skill) random effects; date random effects; best-top-k group aggregation. Click the image to view full-size; the static thumbnail is intentionally compressed.](../results/figures/model_graph_canonical.png){#fig-model-graph .lightbox fig-align="center" width="100%" group="model-graph"}
...
def peak_skill_age(post):
    """Numerical posterior of the age at which S(x) peaks, in years."""
    a_scale = float(post.attrs.get("age_scale", 70.0))
    m = np.exp(post["m0"].values.flatten())
    k = np.exp(post["k0"].values.flatten())
    b = np.exp(post["b0"].values.flatten())
    grid = np.linspace(0.5, 80, 400)
    n = min(2000, len(m))
    sub = np.random.default_rng(0).choice(len(m), n, replace=False)
    M = np.exp(-np.outer(m[sub], grid / a_scale))
    K = np.clip(1 - np.exp(-np.outer(k[sub], grid / a_scale)), 1e-10, 1)
    S = M * K ** b[sub, None]
    return grid[S.argmax(axis=1)]
...
def age_at_half_peak(post):
    ...
    K = np.clip(1 - np.exp(-np.outer(k[sub], grid / a_scale)), 1e-10, 1)
    S = M * K ** b[sub, None]
    half = 0.5 * S.max(axis=1, keepdims=True)
    out = np.empty(S.shape[0])
    for i in range(S.shape[0]):
        idx = np.where(S[i] >= half[i, 0])[0]
        out[i] = grid[idx[0]] if len(idx) else np.nan
    return out
```
...
Drawing observables from the prior (without conditioning on the data) shows the range of behaviour the prior alone admits. This is a useful sanity check: if prior-predictive draws of returns or trip rates are wildly inconsistent with what is biologically plausible, the priors are too diffuse; if they only ever produce plausible patterns, the priors may be over-tight. The checks below mirror the posterior predictive in @fig-ppc but draw from `idata.prior_predictive`. The LOO-PIT panel from the posterior version is omitted (LOO requires a likelihood evaluation that is not defined for prior-only draws).
...
For per-observation predictive accuracy and influential-observation detection we use Pareto-smoothed importance-sampling leave-one-out cross-validation (PSIS-LOO; @vehtari_practical_2017), as implemented by `az.loo`. PSIS-LOO approximates the leave-one-out posterior of each observation by importance-reweighting the full-sample posterior, with a generalised Pareto fit to the upper tail of the importance ratios. The shape parameter $\hat k$ of that Pareto fit is itself a diagnostic: $\hat k < 0.7$ indicates the importance-sampling estimate of the LOO log-density for that observation is reliable; $\hat k > 0.7$ flags observations where the leave-one-out posterior differs substantially from the full posterior, so the estimate may be biased and the observation may also be exerting outsized influence on the fit.
...
The model assumes per-forager daily kcal contributions are conditionally independent given parameters and the date random effect. If a forager's daily Shapley-attributed kcal carries within-forager temporal structure beyond the shared date RE, that would show up as non-zero autocorrelation in the residual sequence. We compute the sample autocorrelation of each forager's daily Shapley-attributed kcal across their consecutive in-camp days (with non-foraging days entered as 0 kcal). Using the in-camp denominator is important here: a forager who comes back with a bonanza one day and decides not to forage the next would induce a negative day-to-day autocorrelation that gets hidden if we only look at consecutive foraging days. Calendar gaps between in-camp stints are not adjusted for, so the ACF is over consecutive in-camp-day index, not raw calendar lag.

```{python}
#| label: fig-shapley-acf
#| fig-cap: "Within-forager autocorrelation of daily Shapley-attributed kcal across consecutive in-camp days (non-foraging days entered as 0 kcal). Coloured lines: per-forager ACFs (one line per forager, n = 49). Black: median ACF across foragers. Shaded band: pointwise 5–95 percentile across foragers. Dashed lines mark $\\pm 2/\\sqrt{\\bar n}$ as a rough 95% reference for the per-forager ACF under the white-noise null, where $\\bar n$ is the median per-forager number of in-camp days."
#| fig-width: 9
#| fig-height: 5
...
The median across foragers is consistent with white noise at all lags 1–7 (the pointwise 5–95 percentile band straddles zero throughout). A handful of individual foragers show |ACF| at small lags above the $\pm 2/\sqrt{\bar n}$ reference, which is the rate expected by chance from $\approx 50$ independent ACF computations and one threshold per lag; we view this as compatible with the model's conditional-independence assumption rather than as evidence of unmodeled within-forager temporal dependence.

### Prior and likelihood sensitivity (power-scaling)

We assess how much the posterior shifts when the prior or likelihood is rescaled, using the power-scaling diagnostic of @kallioinen_detecting_2024 (via `arviz.psens`). The diagnostic perturbs the prior or likelihood by raising it to a power $\alpha$ — yielding a perturbed posterior approximated via Pareto-smoothed importance reweighting — and measures the (symmetrised, normalised) cumulative Jensen–Shannon distance $D_{\text{CJS}}(\pi_\alpha,\pi_1)$ between the perturbed posterior CDF $\pi_\alpha$ and the base posterior $\pi_1$. The reported sensitivity is local at $\alpha = 1$: $D_{\text{CJS}}$ is computed at the two perturbed values $\alpha = 1+\delta$ and $\alpha = 1/(1+\delta)$ — symmetric in $\log\alpha$ around $\alpha=1$ — and the diagnostic is their sum divided by $2\,\log_2(1+\delta)$, with $\delta = 0.01$. Because $D_{\text{CJS}}$ is non-negative and vanishes at $\alpha=1$, this is a magnitude of local response in $\log\alpha$ rather than a signed derivative.
...
Parameters whose prior or likelihood sensitivity rises above the 0.05 reference are listed in the figure above. Because the skill-curve and elasticity priors are weakly informative cross-cultural priors derived from @koster_life_2020 (rather than off-the-shelf vague priors), it is expected that the corresponding posteriors retain non-trivial prior sensitivity even when they are also informed by the data; the prior-vs-posterior overlay in @fig-parameter-forest shows that in each case the posterior nonetheless concentrates inside the prior.

### Comprehensive parameter forest

The figure below shows prior and posterior summaries for every scalar fixed effect in the main-text fit. Parameters are grouped by component (skill curve, effort, success, returns, random-effect scales). Tight posteriors shifted away from the prior indicate parameters informed by the data; posteriors that closely track the prior indicate parameters where the prior dominates.
...
age_grid = np.linspace(0.5, 80, 400)
...
The effort and success components have the same structure as the skill component (Bernoulli with logit link, gender offsets, individual random effects, date random effect) but their age dependence is parametric (effort: quadratic in standardised age; success: forager-skill driven). The fitted curves below summarise their age profiles by gender.
...
Total kcal in the data splits into food returned to camp (`returns.csv`) and food eaten in the field while away from camp (`recall.csv`). The recall stream matters because excluding it underestimates child production in particular [Crittenden et al. 2013]; we keep both in the model so child production is not understated. The pies below decompose the Shapley-attributed kcal into these two channels — first study-wide, then by gender × age class. In-field consumption is concentrated in young males and very young foragers; teen and adult women's recall share is essentially negligible.
...
#| tbl-cap: "Per-forager average kcal per foraging day and per in-camp day (Shapley-attributed), with and without in-field consumption."
...
![Per-forager Shapley-kcal share by resource, stratified by within-gender skill tercile. Two panels (males, females); each forager appears as a faint coloured line, tercile means in bold. Within both genders, high-skill foragers concentrate kcal in palm oil and (males) honey or (females) mela; low-skill foragers spread across maondo, caterpillars, and cassava leaves.](../results/figures/eda_parallel_resource_skill.png){#fig-resource-by-skill fig-align="center" width="100%"}

## Sensitivity Analyses

We compare the fit reported in the main text against an alternative specification using mean skill aggregation in place of best-top-$k$. The substantive conclusions of the paper are preserved. The remaining methodological choice — whether to integrate over age uncertainty — is built into the main-text fit itself (latent age) rather than presented as a sensitivity, because reporting a point-estimate variant would understate posterior uncertainty.
...
#| fig-cap: "Forest plot of the key estimands across the main-text fit and the mean-aggregation sensitivity variant. Points are posterior means; thick bars are 50% credible intervals; thin bars are 95% credible intervals. Substantive conclusions (negative ψ with high posterior probability; skill peak in adulthood) are preserved across both specifications."
#| fig-width: 9
#| fig-height: 7
...
fig, axes = plt.subplots(n_e, 1, figsize=(8, 1.4 * n_e + 1), sharex=False)
if n_e == 1:
    axes = [axes]
...
Each point is one forager's Shapley attribution for one food package, plotted under the two aggregation rules. Points along the dashed $y = x$ line indicate cases where the choice of aggregation rule does not change the attribution. Mean aggregation produces a tail of negative attributions (lower-left quadrant) on packages where a low-skill member dragged down the group mean — best-top-$k$ never produces these because adding any forager can only weakly improve the maximum. Despite these per-package disagreements, the per-forager *totals* across all packages remain positive under both rules.
...
This supplement accompanies the main manuscript. Data and code are available at the project's GitHub repository.
