...
# Display labels for the two gender strata (model coordinates are "male"/"female").
GENDER_DISPLAY = {"male": "Boys/men", "female": "Girls/women"}
...
- Gift exclusion. Food packages recorded as outright gifts from non-camp members (`gift` = 1; 68 returns rows and 2 recall rows) are dropped before kcal computation — gifts are not the recipient's own production. Eight rows flagged as partly gifted (`gift` = 0.5) are retained as recorded; a variant dropping these too is reported in § Sensitivity Analyses.
- Group identifier. Each food package is identified by date and the set of contributing foragers. The `pooled_group` field from the field protocol resolves cases where multiple foragers participated in collecting the same item. We then aggregate to a single `group_id = "<date>_<sorted forager IDs>"`. When the same forager set is recorded against more than one `pooled_group` on the same day — e.g. two sub-trips, or two separately-pooled cooperative items — kcal is summed within the resulting `group_id` and the model treats them as a single observation in the LogNormal returns likelihood. Of the 1 014 `group_id`s in the returns stream, 56 (5.5 %) merge two or more cooperative `pooled_group`s this way (max 7); solo `group_id`s never collide.
- Effort observations. For each forager × in-camp date, an effort indicator is set to 1 if the forager logged positive time on an out-of-camp trip with activity type "Foraging" that day (or contributed to a food package that day), and 0 otherwise. Of the 1 395 forager-days scored as effort = 1, 130 (9.3%) had no foraging-labelled trip that day and are credited solely through the forager's contribution to a food package (129 returned packages; one recall-only record) — food acquired without a foraging-labelled trip that day: gathering on the periphery of camp, or opportunistic collection on outings logged with a non-foraging purpose. Multi-day trips are rare (36 of 2 683 logged trip records) and are credited to the day of departure; subsequent days away are not counted as additional participation days. Trip records logged with a non-foraging purpose (22% of trip records) do not count toward effort in the canonical fit; the all-outings sensitivity variant (§ Sensitivity Analyses) relaxes this. These observations enter the Bernoulli effort component.
...
- Inclusion. Five of 54 listed camp members (three age 0, two age 1) had no foraging activity over the study window and drop out at the time-aggregation step. 49 foragers appear in the foraging-activity log; of those, 48 contributed at least one positive-kcal food package and inform the model. Two of the 49 are missing from the in-camp daily-presence file (`daysincamp.csv`) and so contribute no effort or success observations. One of these two nonetheless contributed to positive-kcal food packages and enters the returns likelihood (he is among the 48); the other went on one logged foraging trip but has no recorded food package, so he occupies a slot in the forager dimension without entering the likelihood. Figures throughout the manuscript and supplement therefore use $n = 48$.
...
#| fig-cap: "kcal distribution of food returned and recalled in the study. 'fish' contains one frog observation; 'game' covers mammals, birds, and reptiles (a turtle, lizards, and snakes); 'cultivated tubers' is mostly cassava; 'cultivated fruit' covers citrus, papaya, cacao, and chilli peppers. BaYaka do not consume domesticated animals."

df_returns_raw = pd.read_csv(here("raw_data/returns.csv"))
df_recall_raw = pd.read_csv(here("raw_data/recall.csv"))
# Canonical gift rule: outright gifts (gift == 1) excluded everywhere.
for _d in (df_returns_raw, df_recall_raw):
    _d.columns = [c.lower() for c in _d.columns]
df_returns_raw = df_returns_raw[pd.to_numeric(df_returns_raw["gift"], errors="coerce").fillna(0) != 1]
df_recall_raw = df_recall_raw[pd.to_numeric(df_recall_raw["gift"], errors="coerce").fillna(0) != 1]
...
Game accounts for just 3.6% of kcal in the study: 57 returned package rows across 29 days, touched by 20 foragers. This is far too sparse to identify a hunting-specific three-parameter learning-senescence curve plus individual random effects, so no hunting-only refit is reported. The near-absence of hunting is itself informative — the age-related gains in production documented here arise almost entirely from gathered and collected resources, so they cannot be attributed to the long-delay proficiency profile that hunting-specific studies emphasize.
...
#| fig-cap: "Log-scale residuals for the LogNormal returns component (one point per group, n = 1 065), plotted against predicted log kcal (left), mean age of contributing foragers (centre), and group size (right). Dashed lines mark $\\pm 2\\hat\\sigma$ from the posterior mean of the LogNormal scale."
#| fig-width: 13
#| fig-height: 4
...
for grp, names in groups_def.items():
    for nm in names:
        ...
        if nm.endswith("_stds"):
            g_lbl = GENDER_DISPLAY["male" if "_male_" in nm else "female"].lower()
            labels = [f"effort RE sd ({g_lbl})", f"skill RE sd ({g_lbl})"]
        elif nm.endswith("_corr"):
            g_lbl = GENDER_DISPLAY["male" if "_male_" in nm else "female"].lower()
            labels = [f"psi ({g_lbl})"]
        elif nm.endswith("_gender"):
            base = nm[:-len("_gender")]
            labels = [f"{base} ({GENDER_DISPLAY[g].lower()})" for g in ("male", "female")]
        else:
            labels = [nm]
...
for g_idx, sex in enumerate(["male", "female"]):
    ...
    axes[0].plot(ages_plot, p_eff.mean(axis=0), color=COLORS_SEX[sex],
                 lw=2.0, label=GENDER_DISPLAY[sex])
...
ax.legend(handles=[
    Patch(facecolor=COLORS_SEX["male"], alpha=0.65, label=GENDER_DISPLAY["male"]),
    Patch(facecolor=COLORS_SEX["female"], alpha=0.65, label=GENDER_DISPLAY["female"]),
...
def _draw_panel(ax, summary, title):
    for sex, color in COLORS_SEX.items():
        ...
        ax.plot(x, y, marker="o", color=color, lw=2.0, label=GENDER_DISPLAY[sex],
...
for ax, df_cv, title in [
    ...
    for sex, color in COLORS_SEX.items():
        ...
        ax.scatter(sub["age"], sub["cv"], s=42, c=color, alpha=0.85,
                   ...
                   label=f"{GENDER_DISPLAY[sex]} (n={len(sub)})")
    ax.set_xlabel("Forager age (years)")
    ax.set_title(title, fontsize=10)
    ax.legend(loc="upper right", frameon=False, fontsize=9)
    for spine in ("top", "right"):
...
# Age class qualifies the gender label: girls/boys under 15, women/men over 20,
# both forms for the 15-20 band.
CLASS_DISPLAY = {
    "female <15": "Girls <15",
    "female 15-20": "Girls/women 15-20",
    "female >20": "Women >20",
    "male <15": "Boys <15",
    "male 15-20": "Boys/men 15-20",
    "male >20": "Men >20",
}
classes = [CLASS_DISPLAY[f"{s} {_age_class(a)}"] for s, a in zip(sex_arr_t, ages_arr)]
class_order = [
    "Girls <15", "Girls/women 15-20", "Women >20",
    "Boys <15", "Boys/men 15-20", "Men >20",
]
...
class_colors = {
    "Girls <15":         "#FCAEAE",
    "Girls/women 15-20": "#E94F37",
    "Women >20":         "#8B1E1E",
    "Boys <15":          "#A6C8E0",
    "Boys/men 15-20":    "#2E86AB",
    "Men >20":           "#1B4F72",
}
...
# Canonical gift rule: outright gifts (gift == 1) excluded; preserve column case.
def _no_gifts_rr(df):
    gcol = next((c for c in df.columns if c.lower() == "gift"), None)
    if gcol is not None:
        df = df[pd.to_numeric(df[gcol], errors="coerce").fillna(0) != 1]
    return df
returns_rr = _no_gifts_rr(returns_rr)
recall_rr = _no_gifts_rr(recall_rr)
...
class_arr_pie = np.array([
    CLASS_DISPLAY_PIE[f"{s} {_ac_pie(a)}"] for s, a in zip(sex_arr_pie, age_arr_pie)
])
...
The same recall/returns decomposition can be applied to the per-forager averages reported above. The "returns only" columns rescale each per-group Shapley share by `returns_kcal / (returns + recall)` for that group; the computation is implemented in `scripts/compute_kcal_summary.py`.

```{python}
#| label: tbl-kcal-summary
#| tbl-cap: "Per-forager average kcal per participation day and per in-camp day (Shapley-attributed), with and without in-field consumption; n = 48 modeled foragers. A participation day is defined exactly as in the model's effort component — a logged foraging trip or credit on a food package that day — and failed trips enter as 0 kcal. One boy has food-package contributions but no daily-presence record: his kcal and package days enter the totals, but he adds no days to the in-camp denominator."
...
ks_disp = ks[[
    ...
    "n_foragers": "n",
    "kcal_per_foraging_day_with_recall":  "kcal/participation day (with recall)",
    "kcal_per_foraging_day_returns_only": "kcal/participation day (returns only)",
    ...
    "n",
    "kcal/participation day (with recall)",
    "kcal/participation day (returns only)",
...
for ax, vals_m, vals_f, ttl in [
    ...
    for j, (vals, color, lbl) in enumerate([
        (vals_m, COL_M_RE, GENDER_DISPLAY["male"]),
        (vals_f, COL_F_RE, GENDER_DISPLAY["female"]),
    ]):
    ...
    ax.set_yticks([1, 0]); ax.set_yticklabels([GENDER_DISPLAY["male"], GENDER_DISPLAY["female"]])
...
#| tbl-cap: "Ratio of between-forager SDs on the observable scale, boys/men relative to girls/women. Values >1 indicate more between-forager variation among boys/men than among girls/women; the right column is the posterior probability that the boys/men SD exceeds the girls/women SD."
...
pd.DataFrame([
    _row("SD ratio: effort deviation, boys/men ÷ girls/women", ratio_dP),
    _row("SD ratio: skill deviation, boys/men ÷ girls/women", ratio_logR),
]).round(3)
```

In words: a typical boy or man one between-individual SD above the boys/men age × gender expectation is roughly $1.32\times$ as skilled as that age-norm and goes on subsistence trips on about $8$ percentage points more days than that norm; for girls/women the equivalent forager is roughly $1.15\times$ as skilled and goes out about $6$ percentage points more often. The skill-scale asymmetry — about three-and-a-half times more between-forager variation among boys/men than among girls/women, with $P(\text{ratio} > 1) \approx 0.92$ — is the more clearly resolved of the two and is the most direct reason $\psi$ is more weakly identified for girls/women than for boys/men: a residual correlation between effort and skill is estimated more precisely when the two random effects vary more across individuals.

Note that the posterior-mean SD ratios in @tbl-re-sd-ratio ($\approx 3.4$ effort, $\approx 3.5$ skill) are means of a heavily right-skewed ratio posterior (95% CIs span roughly 0.5–15) and therefore run well above the typical (median) ratios implied by the deviations quoted above ($\approx 1.4$ and $\approx 2.0$); the direction of the asymmetry, not the magnitude of the posterior-mean ratio, is the robust summary.
...
![Per-forager Shapley-kcal share by resource, stratified by within-gender skill tercile. Two panels (boys/men, girls/women); each forager appears as a faint coloured line, tercile means in bold. Within both genders, high-skill foragers concentrate kcal in oil palm and honey (boys/men) or *mela* (girls/women); low-skill foragers spread across maondo, caterpillars, and cassava leaves.](../results/figures/eda_parallel_resource_skill.png){#fig-resource-by-skill fig-align="center" width="100%"}
...
- **excluding part-gift rows** — the eight rows flagged as partly gifted (gift = 0.5) dropped in addition to the outright gifts excluded canonically. One forager's only food package is among them, so this variant models 47 rather than 48 foragers;
...
SENSITIVITY_VARIANTS = {
    ...
    "sens_no_part_gifts": "excl. part-gift rows",
...
PSIS-LOO comparison of the two aggregation methods on the same observations (n = 1 065 group-level returns):

```{python}
#| label: tbl-aggregation-loo
#| tbl-cap: "PSIS-LOO comparison of the main-text variant (best-top-k) and the mean-aggregation variant. We omit Akaike-style stacking weights — at the magnitude of $\\Delta$ELPD seen here they are binary (1 / 0) and overstate the effective preference; we report `elpd_diff` with its standard error instead, per Vehtari's CV-FAQ."
...
`dse` is the standard error of `elpd_diff`. By the quick rule in Vehtari's [CV-FAQ](https://avehtari.github.io/modelselection/CV-FAQ.html#se_diff), an absolute $\Delta$ELPD below 4 is negligible; above 4 it is judged against its standard error. Here $\Delta$ELPD $= 8.1$ with SE $= 3.6$ ($\approx 2.2$ SE) — a real but modest aggregate preference for mean aggregation. @sivula_uncertainty_2022 caution that this SE can itself be underestimated when the two models make similar predictions, so we read the comparison as suggestive rather than decisive; the pointwise decomposition below and the attribution comparison in the following section show where the difference arises and why the main text retains best-top-$k$.

```{python}
#| label: fig-loo-pointwise-by-kcal
#| fig-cap: "Pointwise LOO ELPD difference (mean aggregation $-$ best-top-$k$) for each of the 1 065 group-level returns, ordered by observed kcal. Positive values: mean aggregation predicts that observation better; negative: best-top-$k$ does. Marker size scales with group size. The two specifications are essentially indistinguishable for small-to-moderate kcal observations (the bulk of the sample); best-top-$k$ predicts the high-kcal *bonanzas* substantially better, mean aggregation predicts modest small-group hauls slightly better."
    ...
    if shap_avg_path.exists():
        ...
        for sex_label, marker in [("male", "o"), ("female", "s")]:
            ...
            sc = ax.scatter(x_avg[m], y_btk[m],
                            ...
                            label=GENDER_DISPLAY[sex_label])
