...
- **excluding largest palm harvest** — only the single exceptionally large ($\approx$ 68 kg) oil-palm harvest removed;
...
Days participating and total time out of camp are highly correlated across foragers (r $\approx$ 0.78), so the binary operationalization preserves the bulk of between-forager variation in time allocation and the effort–skill correlation ψ is not an artifact of discarding duration. Trip durations do rise steeply with age (median $\approx$ 35 min for foragers under 10 vs. $\approx$ 5 h for those 40+), which has a conservative implication for our developmental conclusions: because children's participation days involve fewer hours, their per-day returns *understate* their per-hour efficiency relative to adults, and an hourly-rate analysis would compress, not widen, the inferred age gradient. We retain the binary measure because (i) production is recorded per day and per food package, not per trip-segment, so a kcal-per-hour rate would attribute opportunistic and multi-day acquisition noisily; and (ii) daily production, rather than hourly efficiency, is the quantity that enters a pooled energy budget.

### Production over the study period

If the study itself (announced aims, end-of-stay gifts) had prompted increasing or decreasing foraging over time, the date random effects should trend across the 42-day window. We regress each posterior draw's date random-effect vector (on the linear-predictor scale, i.e. multiplied by its scale parameter) on the centered study day and summarize the posterior of the slope.
...
trend_rows = []
for comp, re_var, sig_var in [
    ("Effort (participation)", "gp_effort_raw", "gp_sigma_effort"),
    ("Success", "gp_success_raw", "gp_sigma_success"),
    ("Returns", "gp_returns_raw", "gp_sigma_returns"),
]:
    re_raw = idata.posterior[re_var].stack(s=("chain", "draw")).values
    sig = idata.posterior[sig_var].stack(s=("chain", "draw")).values
    re = re_raw * sig
